import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import products from '"../../assets/products"'; // Make sure the path is correct
import './SearchBar.css';

export default function SearchBar() {
    const [searchTerm, setSearchTerm] = useState('');
    const [isFocused, setIsFocused] = useState(false); // State for focus
    const navigate = useNavigate();

    const filteredProducts = products.filter((product) =>
        product.title.toLowerCase().includes(searchTerm.toLowerCase())
    );

    const handleDetails = (productId) => {
        navigate(`/product/${productId}`);
        setSearchTerm(''); // Clear search term on navigation
    };

    const handleFocus = () => {
        setIsFocused(true);
    };

    const handleBlur = () => {
        if (searchTerm === '') {
            setIsFocused(false);
        }
    };

    return (
        <div className="search-bar-container">
            <div className="search-bar">
                {isFocused && (
                    <input
                        type="text"
                        placeholder="პროდუქტის ძებნა..."
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        onFocus={handleFocus} // Handle focus
                        onBlur={handleBlur} // Handle blur
                        className={isFocused ? 'focused' : ''} // Apply class on focus
                    />
                )}
                <button onClick={handleFocus} className="btn-search">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className="search-icon">
                        <path d="M416 208c0 45.9-14.9 88.3-40 122.7L502.6 457.4c12.5 12.5 12.5 32.8 0 45.3s-32.8 12.5-45.3 0L330.7 376c-34.4 25.2-76.8 40-122.7 40C93.1 416 0 322.9 0 208S93.1 0 208 0S416 93.1 416 208zM208 352a144 144 0 1 0 0-288 144 144 0 1 0 0 288z"/>
                    </svg>
                </button>
            </div>

            {searchTerm && (
                <div className="search-results">
                    {filteredProducts.map((product) => (
                        <div
                            key={product.id}
                            className="search-result-item"
                            onClick={() => handleDetails(product.id)}
                        >
                            <img src={product.imageUrl} alt={product.title} className="result-image" />
                            <p>{product.title}</p>
                        </div>
                    ))}
                    {filteredProducts.length === 0 && (
                        <div className="no-results">შედეგები არ მოიძებნა</div>
                    )}
                </div>
            )}
        </div>
    );
}
